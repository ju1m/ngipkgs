...
lib.extendMkDerivation {
  ...
  # Explanation: their type is not serializable as an env-var,
  # hence exclude them from mkDerivation args.
  excludeDrvArgNames = [
    "compileFlags"
  ...
  extendDrvArgs =
    finalAttrs:
    {
      ...
      mixFodDeps ? finalAttrs.mixFodDeps or null,
      ...
      mixNixDeps ? finalAttrs.mixNixDeps or { },

      elixir ? finalAttrs.elixir or inputs.elixir,
      erlang ? finalAttrs.erlang or inputs.erlang,
      hex ? finalAttrs.hex or (inputs.hex.override { inherit elixir; }),
      ...
      removeCookie ? finalAttrs.removeCookie or true,
      ...
      stripDebug ? finalAttrs.stripDebug or false,
    ...
    # Explanation: mixNixDeps could be empty when updating them
    #assert mixNixDeps != { } -> mixFodDeps == null;

    # Explanation: cause an infinite recursion here
    #assert stripDebug -> !enableDebugInfo;
    #assert escriptBinName != null -> mixReleaseName == "";

    # Explanation:
    # there's no way to override this with `.override`
    # because every `callPackage` leading to this file will override `.override`,
    # hence pull attributes in `excludeDrvArgNames` from `finalAttrs.passthru` or `initAttrs`.
    let
      mixNixDeps = finalAttrs.passthru.mixNixDeps or initAttrs.mixNixDeps or { };
      erlangCompilerOptions_ = finalAttrs.passthru.erlangCompilerOptions or erlangCompilerOptions;
      compileFlags_ = finalAttrs.passthru.compileFlags or compileFlags;
    in

    {
      #inherit (initAttrs) passthru;
      nativeBuildInputs =
        (initAttrs.nativeBuildInputs or [ ])
        ++
          # Erlang/Elixir deps
          [
      ...
      buildInputs = (initAttrs.buildInputs or [ ]) ++ lib.optionals (escriptBinName != null) [ erlang ];
      ...
      env = {
        ...
        ERL_COMPILER_OPTIONS =
          let
            options = erlangCompilerOptions_ ++ lib.optionals erlangDeterministicBuilds [ "deterministic" ];
          in
      ...
      buildPhase =
        initAttrs.buildPhase or ''
          runHook preBuild

          mix compile --no-deps-check ${lib.concatStringsSep " " compileFlags_}
