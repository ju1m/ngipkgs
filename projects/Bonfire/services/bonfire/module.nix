...
let
  ...
  # ToDo: move entries in sstillUnsafeSecrets to secrets
    ...
    lib.mkMerge [
            ...
            [
              {
                ...
                ExecStart = lib.getExe (
                  pkgs.writeShellApplication {
                    ...
                    text =
                      # Description: load (unsafe) secrets into env-vars
                      # from ${CREDENTIALS_DIRECTORY}/env.* files.
                      lib.concatMapStringsSep "\n" (
                        secret:
                        let
                          credName =
                            if lib.types.path.check cfg.settings.${secret} then
                              "env.${secret}"
                            else
                              cfg.settings.${secret}.name;
                        in
                        lib.optionalString (cfg.settings.${secret} != null) ''
                          ${secret}=''$(systemd-creds cat "${credName}")
                          export ${secret}
                        ''
                      ) stillUnsafeSecrets
                      + ''
      ...
      (lib.mkIf cfg.postgresql.enable {
        ...
        services.postgresql = {
          ...
          extensions = lib.mkIf (lib.elem cfg.package.env.FLAVOUR [ "social" ]) (
